...
export class MarketingService {
  ...
  /**
   * "Dar vida" ao Marketing IA — chamado 1x/dia pelo `MarketingAutoScheduler`
   * (ou sob demanda via `POST /marketing/auto/run-now`). Encadeia o que antes
   * exigia 2 cliques manuais + a geração de imagem de cada post:
   *   1. Analisa tendência dos produtos novos (Trend Hunter + Planner).
   *   2. Gera o próximo dia do calendário — o próprio `MarketingCalendarAgent`
   *      já garante que não repete produto+campanha, então o rodízio entre
   *      produtos acontece sozinho, dia após dia.
   *   3. Gera e anexa uma imagem a cada post novo sem mídia, deixando pronto
   *      pra publicar (manual ou automático, conforme `MARKETING_AUTO_PUBLISH`).
   * Nunca publica nada — só popula o calendário. Isola falha por etapa: se
   * não houver produto analisado ainda, por exemplo, a etapa 2 falha mas a
   * função não lança, só reporta 0 criados (tenta de novo amanhã).
   */
  async autoGenerateDaily(
    ownerId: string,
  ): Promise<{ analyzed: number; created: number; imagesAttached: number }> {
    const { queued: analyzed } = await this.analyzeTrends(ownerId);

    let created = 0;
    try {
      const targetDate = new Date();
      targetDate.setDate(targetDate.getDate() + 1);
      const result = await this.generateCalendar(ownerId, {
        days: 1,
        startDate: targetDate.toISOString(),
      });
      created = result.created;
    } catch (err) {
      this.logger.warn(`Geração automática diária do calendário não pôde rodar: ${String(err)}`);
    }
  ...
  private static readonly AUTO_IMAGE_STYLES = ['premium', 'lifestyle', 'minimalist', 'modern_room'];

  /** Gera e anexa imagem aos posts (draft/scheduled) que ainda não têm mídia nenhuma. */
  private async autoAttachImages(ownerId: string): Promise<number> {
    const pending = await this.posts
      .find({
        ownerId,
        status: { $in: [MarketingPostStatus.DRAFT, MarketingPostStatus.SCHEDULED] },
    ...
    for (const post of pending) {
      try {
        const product = await this.products.findById(post.productId).lean();
        const images = (product?.images ?? {}) as { original?: string };
        if (!images.original) continue;

        const styleKey =
          MarketingService.AUTO_IMAGE_STYLES[attached % MarketingService.AUTO_IMAGE_STYLES.length];
        const url = await this.marketingImage.generate(post.productId, images.original, styleKey);

        const content = { ...(post.content as Record<string, unknown>), mediaUrls: [url] };
        await this.posts.updateOne({ _id: post._id }, { $set: { content } });
        attached++;
      } catch (err) {
        this.logger.warn(`Geração automática de imagem falhou p/ post ${post._id}: ${String(err)}`);
      }
    }
    return attached;
  }
